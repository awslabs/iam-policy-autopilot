...
fn main() {
    ...
    if workspace_embed_dir.exists() {
        fs::remove_dir_all(workspace_embed_dir).expect("Failed to remove existing botocore embed directory");
    }
    
    // Copy the simplified directory to the workspace location
    copy_dir_recursive(&simplified_dir, workspace_embed_dir).expect("Failed to copy botocore simplified data");
    ...
    if workspace_boto3_embed_dir.exists() {
        fs::remove_dir_all(workspace_boto3_embed_dir).expect("Failed to remove existing boto3 embed directory");
    }
    
    // Copy the boto3 directory to the workspace location
    copy_dir_recursive(&boto3_dir, workspace_boto3_embed_dir).expect("Failed to copy boto3 simplified data");
}

fn process_botocore_data(botocore_path: &Path, output_dir: &Path) -> Result<usize, Box<dyn std::error::Error>> {
...
fn find_latest_api_version(service_path: &Path) -> Result<Option<(String, std::path::PathBuf)>, Box<dyn std::error::Error>> {
...
fn process_service_version(version_path: &Path, output_dir: &Path) -> Result<bool, Box<dyn std::error::Error>> {
...
fn process_service_definition(input_path: &Path, output_path: &Path) -> Result<(), Box<dyn std::error::Error>> {
    // Read the original service definition
    let content = fs::read_to_string(input_path)?;
    let original: Value = serde_json::from_str(&content)?;
    
    // Extract version (optional)
    let version = original.get("version").and_then(|v| v.as_str()).map(|s| s.to_string());
...
fn extract_metadata(metadata_value: Option<&Value>) -> Result<ServiceMetadata, Box<dyn std::error::Error>> {
    if let Some(Value::Object(metadata)) = metadata_value {
        let api_version = metadata.get("apiVersion")
            .and_then(|v| v.as_str())
            .ok_or("Missing apiVersion in metadata")?
            .to_string();
        
        let service_id = metadata.get("serviceId")
            .and_then(|v| v.as_str())
...
fn simplify_operations(operations_value: Option<&Value>) -> Result<HashMap<String, SimplifiedOperation>, Box<dyn std::error::Error>> {
...
fn simplify_shapes(shapes_value: Option<&Value>) -> Result<HashMap<String, SimplifiedShape>, Box<dyn std::error::Error>> {
...
fn process_boto3_data(boto3_path: &Path, output_dir: &Path) -> Result<usize, Box<dyn std::error::Error>> {
    let mut processed_count = 0;
...
fn process_boto3_service_version(version_path: &Path, output_dir: &Path) -> Result<bool, Box<dyn std::error::Error>> {
