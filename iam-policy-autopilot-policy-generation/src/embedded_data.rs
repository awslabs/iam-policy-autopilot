...
impl Boto3Resources {
    ...
    pub(crate) fn build_service_versions_map() -> std::collections::HashMap<String, Vec<String>> {
        ...
        let start_time = std::time::Instant::now();
        let mut service_versions: std::collections::HashMap<String, std::collections::HashSet<String>> = std::collections::HashMap::new();
        ...
        log::debug!("Built boto3 service versions map in {:?} (processed {} files, found {} services)",
            duration, file_count, result.len());
...
impl Botocore {
    ...
    pub fn get_waiters(service: &str, api_version: &str) -> Option<std::borrow::Cow<'static, [u8]>> {
    ...
    pub fn get_paginators(service: &str, api_version: &str) -> Option<std::borrow::Cow<'static, [u8]>> {
    ...
    pub(crate) fn build_service_versions_map() -> std::collections::HashMap<String, Vec<String>> {
        ...
        let duration = start_time.elapsed();
        log::debug!("Built service versions map in {:?} (processed {} files, found {} services)",
            duration, file_count, result.len());
...
impl EmbeddedServiceData {
    ...
    pub(crate) async fn get_service_definition(service: &str, api_version: &str) -> Result<SdkServiceDefinition> {
        let data = Botocore::get_service_definition(service, api_version)
            .ok_or_else(|| ExtractorError::validation(format!(
                "Service definition not found for {}/{}", service, api_version
            )))?;
        
        let json_str = std::str::from_utf8(&data)
            .map_err(|e| ExtractorError::validation(format!("Invalid UTF-8 in embedded data: {}", e)))?;
        
        JsonProvider::parse(json_str).await
    ...
    pub fn get_waiters_raw(service: &str, api_version: &str) -> Option<Vec<u8>> {
        Botocore::get_waiters(service, api_version)
    ...
    pub fn get_paginators_raw(service: &str, api_version: &str) -> Option<Vec<u8>> {
        Botocore::get_paginators(service, api_version)
            .map(|data| data.to_vec())
    }
...
mod tests {
    ...
    fn test_build_service_versions_map_returns_hashmap() {
        ...
        for (service, versions) in &service_versions {
            ...
            assert!(!versions.is_empty(), "Service {} should have at least one version", service);
            
            // Versions should be sorted
            let mut sorted_versions = versions.clone();
            sorted_versions.sort();
            assert_eq!(versions, &sorted_versions, "Versions for service {} should be sorted", service);
        }
    }
    ...
    fn test_build_service_versions_map_consistency() {
        ...
        assert_eq!(map1, map2, "build_service_versions_map should return consistent results");
    }
    ...
    fn test_embedded_service_data_build_service_versions_map_delegates() {
        ...
        assert_eq!(embedded_result, botocore_result,
    ...
    async fn test_embedded_service_data_get_service_definition_invalid_service() {
        let result = EmbeddedServiceData::get_service_definition("nonexistent-service", "2023-01-01").await;
        
        assert!(result.is_err(), "Should return error for nonexistent service");
        
        if let Err(e) = result {
            let error_msg = format!("{}", e);
            assert!(error_msg.contains("Service definition not found"),
                "Error should mention service not found: {}", error_msg);
        }
    }
    ...
    fn test_embedded_service_data_get_waiters_raw_invalid_service() {
        ...
        assert!(result.is_none(), "Should return None for nonexistent service");
    }
    ...
    fn test_service_versions_map_structure() {
        ...
        for (service, versions) in &service_versions {
            ...
            assert!(!service.contains('/'), "Service name '{}' should not contain path separators", service);
            assert!(!service.contains('\\'), "Service name '{}' should not contain backslashes", service);
            
            // Versions should look like valid API versions (basic format check)
            for version in versions {
                assert!(!version.is_empty(), "Version should not be empty for service '{}'", service);
                assert!(!version.contains('/'), "Version '{}' should not contain path separators", version);
                assert!(!version.contains('\\'), "Version '{}' should not contain backslashes", version);
            }
        }
    }
    ...
    fn test_service_versions_map_no_duplicates() {
        ...
        for (service, versions) in &service_versions {
            ...
            assert_eq!(versions.len(), unique_versions.len(),
                "Service '{}' should not have duplicate versions", service);
        }
    }
    ...
    fn test_service_versions_map_performance() {
        ...
        assert!(duration.as_secs() < 1,
            "build_service_versions_map took too long: {:?}", duration);
    }
}