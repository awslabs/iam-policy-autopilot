...
use crate::{enrichment::Condition, errors::{ExtractorError, Result}};
use super::{IamPolicy, Statement, Effect};
use regex::Regex;
...
impl PolicyMerger {
    ...
    pub(crate) fn merge_statements(&self, statements: &[Statement]) -> Result<Vec<IamPolicy>> {
        ...
        if policies.len() > 1 {
            log::info!("Created {} policies to stay within size limits", policies.len());
        }
    ...
    fn group_statements_by_mergeable_resources(&self, statements: &[Statement]) -> Result<Vec<ResourceGroup>> {
        // Only process Allow statements for merging
        let allow_statements: Vec<_> = statements.iter()
    ...
    fn would_merged_statement_exceed_limit(&self, group: &ResourceGroup, statement: &Statement) -> Result<bool> {
    ...
    fn can_merge_with_group(&self, statement: &Statement, group: &ResourceGroup) -> Result<bool> {
        ...
        let stmt_conditions_set: std::collections::HashSet<_> = statement.condition.iter().collect();
        ...
        if !self.config.allow_cross_service_merging {
            ...
            let all_services: std::collections::HashSet<_> = stmt_services.union(&group_services).collect();
            if all_services.len() > 1 {
                log::debug!("Cross-service merging not allowed, different services detected: {:?}", all_services);
                return Ok(false);
            }
        }
    ...
    fn get_resource_relationship(&self, resource1: &str, resource2: &str) -> Result<ResourceRelationship> {
        // Exact match
    ...
    fn are_different_resource_types(&self, resource1: &str, resource2: &str) -> bool {
        ...
        let type1 = resource1.split('/').next().unwrap_or(resource1).split(':').next().unwrap_or(resource1);
        let type2 = resource2.split('/').next().unwrap_or(resource2).split(':').next().unwrap_or(resource2);
    ...
    fn check_regex_subsumption(&self, resource1: &str, resource2: &str) -> Result<Option<ResourceRelationship>> {
        ...
        if regex1.is_match(resource2) {
            log::debug!("check_regex_subsumption: {} subsumes {}", resource1, resource2);
        ...
        if regex2.is_match(resource1) {
            log::debug!("check_regex_subsumption: {} subsumes {}", resource2, resource1);
    ...
    fn extract_services_from_actions(&self, actions: &[String]) -> std::collections::HashSet<String> {
        actions.iter()
            .filter_map(|action| {
                action.split(':').next().map(|service| service.to_string())
            })
            .collect()
    }

    /// Extract service names from all actions in a resource group
    fn extract_services_from_group_actions(&self, group: &ResourceGroup) -> std::collections::HashSet<String> {
...
mod tests {
    ...
    fn test_resource_relationship_equivalent() {
        ...
        // Identical resources
        let rel = merger.get_resource_relationship("arn:aws:s3:::bucket/*", "arn:aws:s3:::bucket/*").unwrap();
    ...
    fn test_resource_relationship_subsumption() {
        ...
        let rel = merger.get_resource_relationship("*", "arn:aws:s3:::bucket/*").unwrap();
    ...
    fn test_arn_relationship_prefix_subsumption() {
        ...
        // Prefix subsumption
        let rel = merger.get_arn_relationship(
            "arn:aws:s3:::bucket/*",
    ...
    fn test_no_merge_with_subsumption() {
        ...
        let statements = vec![
            create_test_statement(vec!["s3:GetObject"], vec!["arn:aws:s3:::bucket/*"]),
            create_test_statement(vec!["s3:PutObject"], vec!["arn:aws:s3:::bucket/specific.txt"]),
        ];
    ...
    fn test_merge_incomparable_resources() {
        ...
        let statements = vec![
            create_test_statement(vec!["s3:GetObject"], vec!["arn:aws:s3:::bucket/*"]),
            create_test_statement(vec!["s3:GetAccessPoint"], vec!["arn:aws:s3:us-east-1:123456789012:accesspoint/my-access-point"]),
        ];
    ...
    fn test_no_merge_different_services_when_cross_service_disabled() {
        ...
        let statements = vec![
            create_test_statement(vec!["s3:GetObject"], vec!["arn:aws:s3:::bucket/*"]),
            create_test_statement(vec!["dynamodb:GetItem"], vec!["arn:aws:dynamodb:us-east-1:123456789012:table/MyTable"]),
        ];
        ...
        let s3_statement = merged_policies[0].statements.iter()
        ...
        let dynamodb_statement = merged_policies[0].statements.iter()
    ...
    fn test_merge_multiple_policies() {
        ...
        policy1.add_statement(create_test_statement(vec!["s3:GetObject"], vec!["arn:aws:s3:::bucket1/*"]));
        
        let mut policy2 = IamPolicy::new();
        policy2.add_statement(create_test_statement(vec!["s3:PutObject"], vec!["arn:aws:s3:::bucket1/*"]));
    ...
    fn test_remove_subsumed_resources_no_subsumption() {
        ...
        assert!(result.contains(&"arn:aws:dynamodb:us-east-1:123456789012:table/MyTable".to_string()));
    }
    ...
    fn test_s3_bucket_vs_accesspoint_relationship() {
        ...
        let relationship = merger.get_resource_relationship(bucket_resource, accesspoint_resource).unwrap();
        ...
        let reverse_relationship = merger.get_resource_relationship(accesspoint_resource, bucket_resource).unwrap();
    ...
    fn test_calculate_policy_size() {
        ...
        policy.add_statement(create_test_statement(vec!["s3:GetObject"], vec!["arn:aws:s3:::bucket/*"]));
    ...
    fn test_would_exceed_size_limit() {
        ...
        let mut policy = IamPolicy::new();
        let small_statement = create_test_statement(vec!["s3:GetObject"], vec!["arn:aws:s3:::bucket/*"]);
        
        // Adding a small statement to empty policy should not exceed limit
        assert!(!merger.would_exceed_size_limit(&policy, &small_statement).unwrap());
    ...
    fn test_size_aware_grouping() {
        ...
        let groups = merger.group_statements_by_mergeable_resources(&statements).unwrap();
    ...
    fn test_merge_policies_with_size_limits() {
        ...
        for policy in &merged_policies {
            let size = merger.calculate_policy_size(policy).unwrap();
            if policy.statements.len() > 1 { // Allow single large statements
                assert!(size <= IAM_MANAGED_POLICY_SIZE_LIMIT, "Policy size {} exceeds limit {}", size, IAM_MANAGED_POLICY_SIZE_LIMIT);
            }
        }
    }
    ...
    fn test_would_merged_statement_exceed_limit() {
        ...
        assert!(!merger.would_merged_statement_exceed_limit(&group, &statement2).unwrap());
    }
    ...
    fn test_policy_exceeding_limit_forces_split() {
        ...
        for i in 0..30 {
            ...
            statements.push(create_test_statement(vec!["s3:GetObject"], vec![&long_resource]));
        }
        ...
        for i in 0..20 {
            ...
            statements.push(create_test_statement(vec!["dynamodb:GetItem"], vec![&long_resource]));
        }
        ...
        assert!(policy_size > IAM_MANAGED_POLICY_SIZE_LIMIT,
        ...
        assert!(result.len() > 1, "Large policy should be split into multiple policies, got {}", result.len());
        ...
        for (i, policy) in result.iter().enumerate() {
            ...
            if policy.statements.len() > 1 {
                assert!(size <= IAM_MANAGED_POLICY_SIZE_LIMIT,
                    "Split policy {} has size {} which exceeds limit {}", i, size, IAM_MANAGED_POLICY_SIZE_LIMIT);
            }
            assert!(!policy.statements.is_empty(), "Split policy {} should not be empty", i);
        }
        ...
        assert!(total_statements >= 2, "Should have at least 2 statements after merging (S3 and DynamoDB), got {}", total_statements);
        assert!(total_statements <= 50, "Should not have more statements than we started with, got {}", total_statements);
    }
    ...
    fn test_multiple_large_policies_distribution() {
        ...
        for (i, policy) in policies.iter().enumerate() {
            let size = merger.calculate_policy_size(policy).unwrap();
            assert!(size <= IAM_MANAGED_POLICY_SIZE_LIMIT,
                "Individual policy {} size {} should be under limit {}", i, size, IAM_MANAGED_POLICY_SIZE_LIMIT);
        }
        
        // Create a single merged policy to verify it would exceed the limit
        let all_statements: Vec<Statement> = policies.iter()
        ...
        assert!(merged_size > IAM_MANAGED_POLICY_SIZE_LIMIT,
        ...
        assert!(result.len() > 1, "Should split into multiple policies due to size constraints");
        ...
        for (i, policy) in result.iter().enumerate() {
            ...
            if policy.statements.len() > 1 {
                assert!(size <= IAM_MANAGED_POLICY_SIZE_LIMIT,
                    "Result policy {} has size {} which exceeds limit {}", i, size, IAM_MANAGED_POLICY_SIZE_LIMIT);
            }
        }
        ...
        assert!(total_statements >= 5, "Should have at least 5 statements after merging, got {}", total_statements);
        assert!(total_statements <= original_statements, "Should not have more statements than we started with, got {} vs {}", total_statements, original_statements);
    }
    ...
    fn test_single_statement_exceeding_limit() {
        ...
        assert_eq!(result.len(), 1, "Should return exactly one policy even if it exceeds the limit");
        assert_eq!(result[0].statements.len(), 1, "Should preserve the single statement");
    ...
    fn test_large_statement_grouping_with_size_limits() {
        ...
        for i in 0..20 {
            ...
            statements.push(create_test_statement(
                vec!["lambda:InvokeFunction", "lambda:GetFunction", "lambda:UpdateFunctionCode"],
                vec![&long_resource]
            ));
        }
        ...
        assert!(!groups.is_empty(), "Should create at least one group, got {}", groups.len());
        ...
        for (i, group) in groups.iter().enumerate() {
            ...
            if group.statements.len() > 1 {
                assert!(size <= IAM_MANAGED_POLICY_SIZE_LIMIT,
                    "Group {} merged statement size {} should not exceed limit {}", i, size, IAM_MANAGED_POLICY_SIZE_LIMIT);
            }
        }
        
        // Verify all statements are preserved across groups
        let total_statements: usize = groups.iter().map(|g| g.statements.len()).sum();
        assert_eq!(total_statements, statements.len(), "All statements should be preserved across groups");
    }
    ...
    fn test_catalog_wildcard_does_not_subsume_catalog() {
        ...
        let relationship = merger.get_resource_relationship(catalog_wildcard, catalog_base).unwrap();
        ...
        // Test the reverse relationship too
        let reverse_relationship = merger.get_resource_relationship(catalog_base, catalog_wildcard).unwrap();
        assert_eq!(reverse_relationship, ResourceRelationship::Incomparable,
    ...
    fn test_catalog_wildcard_arn_does_not_subsume_catalog_arn() {
        ...
        let relationship = merger.get_arn_relationship(catalog_wildcard_arn, catalog_base_arn).unwrap();
        ...
        // Test the reverse relationship too
        let reverse_relationship = merger.get_arn_relationship(catalog_base_arn, catalog_wildcard_arn).unwrap();
        assert_eq!(reverse_relationship, ResourceRelationship::Incomparable,
