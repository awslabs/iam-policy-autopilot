...
impl ResourceMatcher {
    ...
    pub(crate) async fn enrich_method_call<'b>(
        ...
        for service_name in &parsed_call.possible_services {
            ...
            if let Some(enriched_call) = self.create_enriched_method_call(
                parsed_call,
                service_name,
    ...
    fn find_operation_fas_map_for_service(
        ...
        self.fas_maps.get(self.service_cfg.rename_service_operation_action_map(service_name).as_ref()).cloned()
    }
    ...
    async fn create_enriched_method_call<'a>(
        ...
        log::debug!("Creating method call for service: {}, and method name: {}", service_name, parsed_call.name);
        ...
        match operation_fas_map_option {
            Some(operation_fas_map) => {
                ...
                let result = operation_fas_map.fas_operations.get(&initial_service_operation_name);
                result
                    .iter().for_each(|additional_operations| { 
        ...
        for operation in operations {
            ...
            let service_reference = service_reference_loader.load(&operation.service(&self.service_cfg)).await?;
            ...
            match service_reference {
                ...
                Some(service_reference) => {
                    ...
                    if let Some(operation_to_authorized_actions) = service_reference_loader.get_operation_to_authorized_actions(&service).await? {
                        log::debug!("Looking up {}", &operation.service_operation_name(&self.service_cfg));
                        if let Some(operation_to_authorized_action) = operation_to_authorized_actions.get(&operation.service_operation_name(&self.service_cfg)) {
                            for action in &operation_to_authorized_action.authorized_actions {
                                let enriched_resources = self.find_resources_for_action_in_service_reference(&action.name, &service_reference)?;
                            ...
                            if let Some(a) = self.create_fallback_action(&parsed_call.name, &service_reference)? { enriched_actions.push(a) }
                        }
                    } else {
    ...
    fn create_fallback_action(
        ...
        let renamed_service = self.service_cfg.rename_service_service_reference(&service_reference.service_name);
        ...
        if !service_reference.actions.contains_key(renamed_action.as_str()) {
            return Ok(None);
        }

        // Look up the action in the Service Reference to find associated resources
        let resources = self.find_resources_for_action_in_service_reference(&action_name, service_reference)?;
        
        Ok(Some(Action::new(action_name.to_string(), resources, vec![])))
    }
    ...
    fn find_resources_for_action_in_service_reference(
        ...
        log::debug!("find_resources_for_action_in_service_reference: service_reference.actions = {:?}", service_reference.actions);
        let mut result = vec![];
        if let Some(action) = service_reference.actions.get(action) {
            ...
            for resource in &action.resources {
                let service_reference_resource = if let Some(r#override) = overrides.and_then(|m| m.get(resource)) {
                    log::debug!("find_resources_for_action_in_service_reference: resource override = {}", r#override);
                    Resource::new(
                        resource.clone(),
                        Some(vec![r#override.clone()]),
                    )
                } else {
                    log::debug!("find_resources_for_action_in_service_reference: looking up resource = {}", resource);
                    log::debug!("find_resources_for_action_in_service_reference: resources = {:?}", service_reference.resources);
                    let arn_patterns = service_reference.resources.get(resource).cloned();
                    log::debug!("find_resources_for_action_in_service_reference: arn_pattern = {:?}", arn_patterns);
                    Resource::new(
                        resource.clone(),
                        arn_patterns,
                    )
...
mod tests {
    ...
    async fn test_enrich_method_call() {
        ...
        let (_, service_reference_loader) = mock_remote_service_reference::setup_mock_server_with_loader().await;
        ...
        tokio::fs::write(&s3_action_file, s3_action_json).await.unwrap();
        

        let result = matcher.enrich_method_call(&parsed_call, &service_reference_loader).await;
    ...
    async fn test_fallback_for_service_without_operation_action_map() {
        ...
        let service_cfg = ServiceConfiguration {
            rename_services_operation_action_map: [("mediastore-data".to_string(), "mediastore".to_string())].iter().cloned().collect(),
            rename_services_service_reference: [("mediastore-data".to_string(), "mediastore".to_string())].iter().cloned().collect(),
            rename_operations: [("s3:ListObjectsV2".to_string(), crate::service_configuration::OperationRename {
        ...
        let (mock_server, loader) = mock_remote_service_reference::setup_mock_server_with_loader().await;
        ...
        assert!(result.is_ok(), "Fallback enrichment should succeed: {:?}", result);
    ...
    async fn test_error_for_missing_operation_action_map_when_required() {
        ...
        let result = matcher.enrich_method_call(&parsed_call, &loader).await;
        assert!(result.is_ok(), "Should succeed with fallback action when operation action map is missing");
        
        let enriched_calls = result.unwrap();
        assert_eq!(enriched_calls.len(), 1, "Should have one enriched call using fallback");
        assert_eq!(enriched_calls[0].method_name, "get_object");
        assert_eq!(enriched_calls[0].service, "s3");

        // This below assertion fails intermittently, so adding this println here
        assert_eq!(
            enriched_calls[0].actions.len(), 1, 
            "Should have one fallback action, enriched_calls[0].action is: {:?}", enriched_calls[0].actions);
        
        let action = &enriched_calls[0].actions[0];
        assert_eq!(action.name, "s3:GetObject", "Should use fallback action name");
    }
    ...
    async fn test_enrich_method_call_returns_empty_vec_for_missing_operation() {
        ...
        let service_cfg = ServiceConfiguration {
            rename_services_operation_action_map: [("connectparticipant".to_string(), "execute-api".to_string())].iter().cloned().collect(),
            rename_services_service_reference: [("connectparticipant".to_string(), "execute-api".to_string())].iter().cloned().collect(),
            rename_operations: HashMap::new(),
            resource_overrides: HashMap::new(),
        };
        ...
        // Assertions
        assert!(result.is_ok(), "enrich_method_call should succeed even when no operations match");
        
        let enriched_calls = result.unwrap();
        assert_eq!(enriched_calls.len(), 0, "Explicit check: enriched calls length should be 0");
        
        println!("✓ Test passed: enrich_method_call correctly returns empty Vec for missing operation");
    }
    ...
    async fn test_resource_overrides_for_iam_get_user() {
        ...
        let (mock_server, service_reference_loader) = mock_remote_service_reference::setup_mock_server_with_loader().await;

        mock_remote_service_reference::mock_server_service_reference_response(&mock_server, "iam", serde_json::json!({
                    "Name": "iam",
                    "Resources": [
                        {
                            "Name": "user",
        ...
        // Test the enrichment
        let result = matcher.enrich_method_call(&parsed_call, &service_reference_loader).await;
        assert!(result.is_ok(), "Enrichment should succeed for iam:GetUser with resource override");
        ...
        assert!(resource.arn_patterns.is_some(), "Resource should have ARN patterns");
        let arn_patterns = resource.arn_patterns.as_ref().unwrap();
        assert_eq!(arn_patterns.len(), 1, "Should have exactly one ARN pattern from override");
        assert_eq!(arn_patterns[0], "*", "Resource override should be '*' for iam:GetUser user resource");
        
        println!("✓ Test passed: iam:GetUser correctly uses resource override '*' for user resource");
    }
    ...
    async fn test_resource_overrides_mixed_with_normal_resources() {
        ...
        // Test the enrichment
        let result = matcher.enrich_method_call(&parsed_call, &service_reference_loader).await;
        assert!(result.is_ok(), "Enrichment should succeed for s3:GetObject with mixed overrides");
        ...
        let bucket_resource = action.resources.iter().find(|r| r.name == "bucket").unwrap();
        let object_resource = action.resources.iter().find(|r| r.name == "object").unwrap();
        ...
        assert_eq!(bucket_patterns[0], "arn:aws:s3:::*", "Bucket should use override value");
        ...
        assert_eq!(object_patterns[0], "arn:${Partition}:s3:::${BucketName}/${ObjectName}", "Object should use normal service reference");
