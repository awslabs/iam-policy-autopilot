            ...
            for operation in &mut operations {
                ...
                if operation.authorized_actions.is_empty() {
                    ...
                    for authorized_action in &mut operation.authorized_actions {
                        authorized_action.name = format!("{}:{}", authorized_action.service.to_lowercase(), authorized_action.name);
                    }
                }
            }
        }
...
fn deserialize_actions_map<'de, D>(deserializer: D) -> std::result::Result<HashMap<String, Action>, D::Error>
where
    D: Deserializer<'de>,
{
    #[derive(Deserialize)]
...
fn deserialize_resources_map<'de, D>(deserializer: D) -> std::result::Result<HashMap<String, Vec<String>>, D::Error>
where
    D: Deserializer<'de>,
{
...
fn deserialize_service_reference_mapping(value: Value) -> crate::errors::Result<HashMap<String, Url>> {
    #[derive(Deserialize)]
    struct ServiceEntry {
        service: String,
        url: String,
    ...
    for entry in entries {
        let url = Url::parse(&entry.url).map_err(|e| ExtractorError::service_reference_parse_error_with_source("RemoteServiceReferenceLoaderMappingInitialization", "Failed to parse service reference mapping", e))?;
...
impl RemoteServiceReferenceLoader {
    ...
    async fn get_or_init_mapping(&self) -> crate::errors::Result<&ServiceReferenceMapping> {
        self.service_reference_mapping.get_or_try_init(|| async {
            let json_text = self.client
                .get(&self.mapping_url)
                .send()
                .await
                .map_err(|e| ExtractorError::service_reference_parse_error_with_source("RemoteServiceReferenceLoaderMappingInitialization", "Failed to send request".to_string(), e))?
                .error_for_status()
                .map_err(|e| ExtractorError::service_reference_parse_error_with_source("RemoteServiceReferenceLoaderMappingInitialization", "Failed to fetch mapping".to_string(), e))?
                .text()
                .await
                .map_err(|e| ExtractorError::service_reference_parse_error_with_source("RemoteServiceReferenceLoaderMappingInitialization", "Failed to read response".to_string(), e))?;
            
            let json_value: serde_json::Value = serde_json::from_str(&json_text)
                .map_err(|e| ExtractorError::service_reference_parse_error_with_source("RemoteServiceReferenceLoaderMappingInitialization", "Failed to parse JSON".to_string(), e))?;
            
            let mapping = deserialize_service_reference_mapping(json_value)
                .map_err(|e| ExtractorError::service_reference_parse_error_with_source("RemoteServiceReferenceLoaderMappingInitialization", "Failed to deserialize mapping".to_string(), e))?;
    ...
    fn create_client() -> crate::errors::Result<Client> {
        ...
        Client::builder()
            ...
            .build()
            .map_err(|e| ExtractorError::service_reference_parse_error_with_source("RemoteServiceReferenceLoaderClientInitialization", "Failed to create service reference client".to_string(), e))
    }
        ...
        if let Ok(metadata) = fs::metadata(path).await {
            if let Ok(modified) = metadata.modified() {
                if let Ok(elapsed) = SystemTime::now().duration_since(modified) {
                    return elapsed < Duration::from_secs(DEFAULT_FILE_SYSTEM_CACHE_DURATION_IN_SECONDS);
                }
            }
        }
        false
    ...
    pub(crate) async fn load(&self, service_name: &str) -> crate::errors::Result<Option<ServiceReference>> {
        ...
        if !self.disable_file_system_cache
            && Self::is_cache_valid(&cache_path).await {
                if let Ok(content) = fs::read_to_string(&cache_path).await {
                    if let Ok(service_ref) = JsonProvider::parse::<ServiceReference>(&content).await {
                        self.service_cache.write().await.insert(service_name.to_string(), service_ref.clone());
        ...
        match service_url {
            Some(service_url) => {
                let service_reference_content = self.client
                .get(service_url.as_ref())
                .send()
                .await
                .map_err(|e| ExtractorError::service_reference_parse_error_with_source(service_name, "Failed to fetch service reference data".to_string(), e))?
                .text()
                .await
                .map_err(|e| ExtractorError::service_reference_parse_error_with_source(service_name, "Failed to read service reference response".to_string(), e))?;

                let service_ref: ServiceReference = JsonProvider::parse(&service_reference_content).await
                    .map_err(|e| ExtractorError::service_reference_parse_error_with_source(
                        service_name,
    ...
    pub(crate) async fn get_operation_to_authorized_actions(&self, service_name: &str) -> crate::errors::Result<Option<OperationToAuthorizedActionMap>> {
        Ok(self.load(service_name).await?.and_then(|sr| sr.operation_to_authorized_actions))
    }
...
mod tests {
    ...
    async fn test_get_cache_path() {
        ...
        assert!(cache_path.ends_with("IAMPolicyAutopilot/s3.json") || cache_path.ends_with("IAMAutoPilot\\s3.json"));
    }
