...
impl Boto3ResourcesRegistry {
    ...
    pub fn load_common_services_with_utilities() -> Self {
        ...
        let common_services = match extract_services_from_embedded_utilities_mapping() {
            ...
            Err(e) => {
                log::warn!("Failed to extract services from embedded utilities mapping: {}", e);
                vec![]
            }
        };
    ...
    pub fn find_services_for_resource(&self, resource_type: &str) -> Vec<String> {
        self.resource_to_services.get(resource_type)
            .cloned()
            .unwrap_or_default()
    }
...
impl Boto3ResourcesModel {
    ...
    pub fn load_from_embedded(service_name: &str) -> Result<Self, String> {
        ...
        let versions = service_versions.get(service_name)
            .ok_or_else(|| format!("Service '{}' not found in embedded boto3 data", service_name))?;
        ...
        let resources_data = EmbeddedBoto3Data::get_resources_raw(service_name, latest_version)
            .ok_or_else(|| format!("Resources data not found for {}/{}", service_name, latest_version))?;
            ...
            for resource_type in service_utilities.resource_methods.keys() {
                if let Some(resource_def) = model.resource_types.get(resource_type) {
                    if !model.service_constructors.contains_key(resource_type) {
                        ...
                        model.service_constructors.insert(resource_type.clone(), constructor_spec);
                    }
                }
            }
        }
    ...
    fn parse_resources_content(service_name: &str, content: &str) -> Result<Self, String> {
        let json: Boto3ResourcesJson = serde_json::from_str(content)
    ...
    fn parse_service_constructors(
        ...
        if let Some(has) = service.has {
            for (constructor_name, has_spec) in has {
                let identifiers_count = has_spec.resource.identifiers
                    .as_ref()
                ...
                model.service_constructors.insert(constructor_name, constructor_spec);
            }
        }
        ...
        if let Some(has_many_specs) = service.has_many {
            for (collection_name, has_many_json) in has_many_specs {
                ...
                let identifier_params = has_many_json.request.params
                    .unwrap_or_default()
                    .into_iter()
                ...
                model.service_has_many.insert(snake_case_name, has_many_spec);
            }
        }
    ...
    fn parse_resource_actions(
        ...
        if let Some(resource_actions) = resource_actions {
            for (action_name, action_spec) in resource_actions {
                let identifier_params = action_spec.request.params
                    .unwrap_or_default()
                    .into_iter()
                    .filter(|p| p.source == "identifier")
                    .collect();
    ...
    fn parse_special_operations(
        ...
        if let Some(load_spec) = &resource_spec.load {
            let identifier_params = load_spec.request.params
                .clone()
                .unwrap_or_default()
                .into_iter()
                .filter(|p| p.source == "identifier")
        ...
        if let Some(waiters) = &resource_spec.waiters {
            for (waiter_name_pascal, waiter_spec) in waiters {
                ...
                let identifier_params = waiter_spec.params
                    .clone()
                    .unwrap_or_default()
                    .into_iter()
                    .filter(|p| p.source == "identifier")
    ...
    pub fn get_action_mapping(&self, resource_type: &str, action_name: &str) -> Option<&ActionMapping> {
    ...
    pub fn get_resource_utility_method(&self, resource_type: &str, method_name: &str) -> Option<&ResourceUtilityMethod> {
    ...
    pub fn get_has_many_spec(&self, resource_type: &str, collection_name: &str) -> Option<&HasManySpec> {
        let resource_def = self.resource_types.get(resource_type)?;
...
mod tests {
    ...
    fn test_load_dynamodb_model_from_embedded() {
        ...
        if result.is_ok() {
            ...
            assert!(table_def.actions.contains_key("GetItem") ||
    ...
    fn test_load_s3_model_from_embedded() {
        ...
        if result.is_ok() {
            ...
            assert!(bucket_def.actions.contains_key("Delete") ||
    ...
    fn test_embedded_utilities_mapping_access() {
        ...
        if result.is_ok() {
            ...
            assert!(!services.is_empty(), "Should extract at least one service from utilities mapping");
            
            // Check for expected services
            assert!(services.contains(&"s3".to_string()), "Should contain s3 service");
            assert!(services.contains(&"ec2".to_string()), "Should contain ec2 service");
            assert!(services.contains(&"dynamodb".to_string()), "Should contain dynamodb service");
        }
