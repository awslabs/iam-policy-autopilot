...
impl<'a> PaginatorExtractor<'a> {
    ...
    pub(crate) fn extract_paginate_method_calls(
        ...
        for paginate_call in paginate_calls {
            if let Some((paginator, paginator_idx)) = self.match_paginate_to_paginator_with_index(&paginate_call, &paginators) {
    ...
    fn find_chained_paginator_calls(
        ...
        let chained_pattern = "$CLIENT.get_paginator($OPERATION $$$GET_ARGS).paginate($$$PAGINATE_ARGS)";
    ...
    fn create_fallback_synthetic_method_call(&self, paginator_info: &PaginatorInfo) -> SdkMethodCall {
        // Convert paginator operation name to match method lookup index format
        let method_name = self.convert_paginator_operation_to_method_name(&paginator_info.operation_name);
        
        // Look up all services that provide this method
        let possible_services = if let Some(service_refs) = self.service_index.method_lookup.get(&method_name) {
            service_refs.iter()
    ...
    fn create_chained_synthetic_method_call(&self, chained_call: &ChainedPaginatorCallInfo) -> SdkMethodCall {
        // Convert paginator operation name to match method lookup index format
        let method_name = self.convert_paginator_operation_to_method_name(&chained_call.operation_name);
...
mod tests {
    ...
    fn create_test_service_index() -> ServiceModelIndex {
        use crate::extraction::sdk_model::{SdkServiceDefinition, ServiceMetadata, Operation, Shape, ServiceMethodRef};
        ...
        list_objects_members.insert("Bucket".to_string(), crate::extraction::sdk_model::ShapeReference {
            shape: "String".to_string(),
        });
        list_objects_members.insert("Prefix".to_string(), crate::extraction::sdk_model::ShapeReference {
            shape: "String".to_string(),
        });
        ...
        method_lookup.insert("list_objects_v2".to_string(), vec![
            ServiceMethodRef {
...
"#;
        ...
        for call in &paginate_calls {
            ...
            assert_eq!(call.metadata.as_ref().unwrap().receiver, Some("client".to_string()));
        }
        
        // Should have different arguments
        let bucket_values: Vec<String> = paginate_calls.iter()
            .filter_map(|call| {
                call.metadata.as_ref().and_then(|meta| {
                    meta.parameters.iter().find_map(|param| {
                        if let Parameter::Keyword { name, value, .. } = param {
                            if name == "Bucket" { Some(value.as_string().to_string()) } else { None }
                        } else { None }
                    })
                })
            })
    ...
    fn test_extract_quoted_string() {
        ...
        assert_eq!(extractor.extract_quoted_string("'list_objects_v2'"), Some("list_objects_v2".to_string()));
        assert_eq!(extractor.extract_quoted_string("\"list_objects_v2\""), Some("list_objects_v2".to_string()));
        assert_eq!(extractor.extract_quoted_string(" 'list_objects_v2' "), Some("list_objects_v2".to_string()));
        
        // Test unquoted (edge case)
        assert_eq!(extractor.extract_quoted_string("operation_name"), Some("operation_name".to_string()));
    }
...
"#;
        ...
        let call = &paginate_calls[0];
        let param_names: Vec<String> = call.metadata.as_ref().unwrap().parameters.iter()
            .filter_map(|param| {
                if let Parameter::Keyword { name, .. } = param {
                    Some(name.clone())
                } else { None }
