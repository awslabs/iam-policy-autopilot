...
use crate::extraction::python::boto3_resources_model::{Boto3ResourcesModel, Boto3ResourcesRegistry, HasManySpec, OperationType};
...
impl<'a> ResourceDirectCallsExtractor<'a> {
    ...
    pub(crate) fn extract_resource_method_calls(
        ...
        let resource_analyses = self.analyze_resource_usage(&constructors, &method_calls, self.registry.models());
        ...
        for analysis in resource_analyses {
            ...
            match analysis.usage {
                ...
                ResourceUsage::HasUnmatchedMethod { matched_calls, unmatched_method } => {
        ...
        let collection_synthetics = self.find_and_generate_collection_synthetics(ast, &constructors);
    ...
    fn analyze_resource_usage(
        ...
        for constructor in constructors {
            ...
            let resource_methods: Vec<_> = method_calls.iter()
            ...
            for method_call in resource_methods {
                ...
                let utility_calls = self.try_expand_resource_utility_method(method_call, constructor, boto3_model);
    ...
    fn try_match_method(
        ...
        if let Some(call) = self.create_synthetic_method_call_with_waiter_resolution(method_call, constructor, boto3_model) {
            return Some(call);
        }
    ...
    fn try_expand_resource_utility_method(
        ...
        let utility_method = match boto3_model.get_resource_utility_method(
        ...
        for operation in &utility_method.operations {
            ...
            for id_mapping in &utility_method.identifier_mappings {
                if let Some(constructor_arg) = constructor.constructor_args.get(id_mapping.constructor_arg_index) {
                    let value = match constructor_arg {
                        Parameter::Positional { value, .. } => value.clone(),
                        Parameter::Keyword { value, .. } => value.clone(),
                        Parameter::DictionarySplat { expression, .. } => ParameterValue::Unresolved(expression.clone()),
                    };
            ...
            for (arg_index, param) in method_call.arguments.iter().enumerate() {
                ...
                let param_to_add = if let Parameter::Positional { value, type_annotation, .. } = param {
                    ...
                    match param {
                        Parameter::Keyword { name, value, type_annotation, .. } => {
            ...
            for required_param in &operation.required_params {
                ...
                let param_exists = parameters.iter().any(|p| {
                    matches!(p, Parameter::Keyword { name, .. } if name == required_param)
                });
                
                if !param_exists {
                    parameters.push(Parameter::Keyword {
                        name: required_param.clone(),
                        value: ParameterValue::Unresolved(format!("synthetic_{}", required_param.to_case(Case::Snake))),
                        position: parameters.len(),
                        type_annotation: None,
                    });
                }
    ...
    fn generate_synthetic_calls_for_resource(
        ...
        let (start_pos, end_pos) = match evidence {
            SyntheticEvidenceSource::UnmatchedMethod(ref method_call) => (
                method_call.start_position,
                method_call.end_position,
            ),
        };
        ...
        for action_mapping in resource_def.actions.values() {
            ...
            for param_mapping in &action_mapping.identifier_params {
                if let Some(param_name) = &param_mapping.name {
                    if let Some(_identifier) = resource_def.identifiers.iter()
    ...
    fn find_resource_method_calls(
        ...
        method_calls.sort_by(|a, b| {
            a.resource_var.cmp(&b.resource_var)
    ...
    fn create_synthetic_method_call_with_waiter_resolution(
        ...
        let action_mapping = boto3_model.get_action_mapping(
            &constructor.resource_type,
            &method_call.method_name,
        )?;
        ...
        let resolved_operation = match &action_mapping.operation {
            OperationType::Waiter { waiter_name } => {
                ...
                if let Some(service_def) = self.service_index.services.get(&constructor.service_name) {
                    if let Some(operation) = service_def.waiters.get(waiter_name) {
                        operation.name.to_case(Case::Snake)
                    } else {
                        log::debug!("Waiter '{}' not found in service '{}' waiters", waiter_name, constructor.service_name);
                        return None;
                    }
                } else {
                    log::debug!("Service '{}' not found in ServiceModelIndex", constructor.service_name);
                    return None;
                }
            }
            OperationType::SdkOperation(op_name) | OperationType::Load(op_name) => {
        ...
        for (i, param) in method_call.arguments.iter().enumerate() {
            let adjusted_param = match param {
                Parameter::Keyword { name, value, type_annotation, .. } => Parameter::Keyword {
                    name: name.clone(),
                    value: value.clone(),
                    position: combined_parameters.len() + i,
                    type_annotation: type_annotation.clone(),
                },
                Parameter::Positional { value, type_annotation, .. } => Parameter::Positional {
                    value: value.clone(),
                    position: combined_parameters.len() + i,
                    type_annotation: type_annotation.clone(),
                },
    ...
    fn find_and_generate_collection_synthetics(
        ...
        for pattern in patterns {
            for node_match in root.find_all(pattern) {
                ...
                let constructor = match constructors.iter().find(|c| c.variable_name == resource_var) {
                    Some(c) => c,
                    None => continue,
                };
                ...
                if let Some(has_many_spec) = boto3_model.get_has_many_spec(&constructor.resource_type, &attr_name) {
        ...
        for pattern in patterns {
            for node_match in root.find_all(pattern) {
                ...
                for (service_name, boto3_model) in self.registry.models() {
                    ...
                    for resource_def in boto3_model.get_all_resource_definitions().values() {
                        if let Some(has_many_spec) = resource_def.has_many.get(&attr_name) {
                            ...
                            calls.push(SdkMethodCall {
                                ...
                                metadata: Some(SdkMethodCallMetadata {
                                    parameters: self.generate_synthetic_parameters(&has_many_spec.identifier_params),
                                    return_type: None,
                                    start_position: (start.line() + 1, start.column(node) + 1),
                                    end_position: (node.end_pos().line() + 1, node.end_pos().column(node) + 1),
                    ...
                    if let Some(service_has_many_spec) = boto3_model.get_service_has_many_collections().get(&attr_name) {
                        ...
                        calls.push(SdkMethodCall {
                            ...
                            possible_services: vec![service_name.clone()],
                            metadata: Some(SdkMethodCallMetadata {
                                parameters: self.generate_synthetic_parameters(&service_has_many_spec.identifier_params),
                                return_type: None,
                                start_position: (start.line() + 1, start.column(node) + 1),
                                end_position: (node.end_pos().line() + 1, node.end_pos().column(node) + 1),
                                receiver: Some(receiver_var.clone()),  // Use actual receiver from code
    ...
    fn generate_synthetic_parameters(&self, param_mappings: &[crate::extraction::python::boto3_resources_model::ParamMapping]) -> Vec<Parameter> {
        param_mappings.iter().enumerate().map(|(i, mapping)| {
            Parameter::Keyword {
                name: mapping.target.clone(),
                value: ParameterValue::Unresolved(format!("synthetic_{}", mapping.target.to_case(Case::Snake))),
                position: i,
                type_annotation: None,
            }
        }).collect()
