...
mod tests {
    ...
    // =========================================================================
    // TELEMETRY.md validation — every record_result_* key in production code
    // must be documented in the "Result Data" section of TELEMETRY.md.
    // =========================================================================

    #[test]
    fn test_span_result_fields_documented_in_telemetry_md() {
        ...
        // Scan all .rs files in the workspace for record_result_* calls
        ...
        let patterns = [
            regex::Regex::new(r#"record_result_str\(\s*"([^"]+)""#).unwrap(),
            regex::Regex::new(r#"record_result_number\(\s*"([^"]+)""#).unwrap(),
            regex::Regex::new(r#"record_result_set\(\s*"([^"]+)""#).unwrap(),
        ];
            ...
            })
        {
            ...
            // Truncate at #[cfg(test)] to exclude inline test modules
        ...
        for key in &recorded_keys {
            assert!(
                telemetry_md.contains(&format!("| `{key}` |")),
                "TELEMETRY.md missing result field documentation for `{key}`. \
                 Every record_result_* key must have a row in the Result Data table."
            );
        }
    }
}
