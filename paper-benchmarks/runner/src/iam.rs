...
use crate::types::{RoleInfo, RunPolicies};

// ---------------------------------------------------------------------------
// IAM execution role management
// ---------------------------------------------------------------------------

/// Number of times to retry `iam:AttachRolePolicy` for managed ARNs.
/// Newly created roles are subject to IAM eventual consistency.
const MAX_ATTACH_ATTEMPTS: u32 = 5;

/// Create a temporary IAM role and attach the supplied [`RunPolicies`].
/// Returns [`RoleInfo`] on success.
///
/// - [`RunPolicies::InlineDocuments`] merges all documents into a single inline
///   role policy (`iam:PutRolePolicy`).
/// - [`RunPolicies::ManagedArns`] attaches the supplied AWS managed-policy ARNs
///   directly (`iam:AttachRolePolicy`) without creating any new policies.
///
...
pub(crate) async fn create_execution_role(
    ...
    // ── Attach policies ───────────────────────────────────────────────────────
    //
    // For InlineDocuments we use iam:PutRolePolicy (inline role policies) rather
    // than iam:CreatePolicy + iam:AttachRolePolicy. Inline policies:
    ...
    //   • Are deleted automatically when the role is deleted.
    //   • Support up to 10,240 bytes per inline policy document.
    //
    // For ManagedArns we attach the supplied ARNs directly with retry to
    // tolerate IAM eventual consistency on the freshly created role.
    let (policy_names, policy_arns) = match policies {
        RunPolicies::InlineDocuments(policy_docs) => {
            let policy_names =
                attach_inline_policy(iam, &role_name, role_suffix, &policy_docs).await?;
            (policy_names, vec![])
        }
        RunPolicies::ManagedArns(arns) => {
            attach_managed_arns(iam, &role_name, &arns).await?;
            (vec![], arns)
        }
    };
...
/// Merge `policy_docs` into a single inline role policy and attach it via
/// `iam:PutRolePolicy`. Returns the inline policy name(s) attached (empty when
...
async fn attach_inline_policy(
    ...
    // Merge all statements from every document. When combining multiple policy
    // documents into one, duplicate Sid values would cause IAM to reject the
    // merged document with MalformedPolicyDocument. Sids are optional
    // identifiers that carry no semantic weight for permission evaluation, so
    // we strip them during the merge.
    ...
    Ok(vec![inline_policy_name])
}

/// Attach the supplied AWS managed-policy ARNs to `role_name` via
/// `iam:AttachRolePolicy`, retrying each attach to tolerate IAM eventual
/// consistency on a freshly created role.
async fn attach_managed_arns(iam: &IamClient, role_name: &str, arns: &[String]) -> Result<()> {
    for policy_arn in arns {
        ...
        for attempt in 0..MAX_ATTACH_ATTEMPTS {
                ...
                .await
            {
                ...
                Err(e) => {
                    let wait = 2u64.pow(attempt); // 1, 2, 4, 8, 16 s
                    warn!(
                        "[IAM] iam:AttachRolePolicy attempt {}/{} failed for {} on role {} — \
                         retrying in {}s ...",
                        attempt + 1,
                        MAX_ATTACH_ATTEMPTS,
                        policy_arn,
        ...
        if let Some(e) = last_err {
            return Err(anyhow::anyhow!(
                "iam:AttachRolePolicy failed for {policy_arn} on role {role_name}: {e:#}"
            ));
        }
        info!("[IAM] Attached managed policy {} to role {}", policy_arn, role_name);
    }
    Ok(())
}
...
pub async fn create_deploy_role(
    ...
    create_execution_role(
        iam,
        RunPolicies::InlineDocuments(vec![deploy_policy]),
        role_suffix,
        account,
        caller_arn,
    )
...
/// Delete the execution role, detaching managed policies and deleting inline
/// policies first.
///
/// IAM requires all attached managed policies to be detached and all inline
/// policies to be removed before `delete_role` will succeed.
pub async fn cleanup_execution_role(iam: &IamClient, role_info: &RoleInfo) {
    info!("[IAM] Cleaning up role {} ...", role_info.role_name);
    // Detach any managed policies attached to the role (ManagedArns path).
    for policy_arn in &role_info.policy_arns {
        let _ = iam
            .detach_role_policy()
