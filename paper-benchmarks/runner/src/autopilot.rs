...
/// Build a [`Command`] that invokes a pre-built `iam-policy-autopilot` binary
/// directly (rather than via `cargo run`).
///
/// The benchmark pipeline generates policies for many scripts in a loop and is
/// given the path to an already-built release binary (see `--autopilot-binary`),
/// so it does not want the `cargo run` freshness check on every invocation.
fn autopilot_binary_command(autopilot_binary: &str, args: &[&str]) -> Command {
    let mut cmd = Command::new(autopilot_binary);
    cmd.args(args);
    cmd
}

/// Parse the `{"Policies": [{"Policy": …}, …]}` envelope emitted by
/// `generate-policies` into a `Vec` of policy documents.
///
/// Returns `None` (with a logged error) if the output cannot be parsed or the
/// resulting list is empty.
fn parse_generate_policies_output(stdout: &[u8]) -> Option<Vec<Value>> {
    let parsed: AutopilotPoliciesOutput = match serde_json::from_slice(stdout) {
        Ok(v) => v,
        Err(e) => {
    ...
    let policies: Vec<Value> = parsed.policies.into_iter().map(|item| item.policy).collect();
...
/// Run a pre-built `iam-policy-autopilot generate-policies` binary against a
/// single script and return the generated IAM policy documents.
///
/// This is the public entry point used by the benchmark pipeline, which is
/// given the path to an already-built release binary via `--autopilot-binary`.
/// Returns `None` on any spawn/parse failure or when no policies are produced.
#[must_use]
pub fn generate_policies(
    ...
    info!(
        "[autopilot] Generating IAM policies via {}: generate-policies --region {} --account {} {}",
        autopilot_binary, region, account, script_str
    );

    let output = autopilot_binary_command(
        autopilot_binary,
        &[
            "generate-policies",
            "--region",
    ...
    match output {
        ...
        Ok(out) => parse_generate_policies_output(&out.stdout),
    }
}
...
///
/// Used by the in-process [`crate::execution::run_language`] pipeline, which
/// rebuilds the binary on demand. The benchmark pipeline uses the pre-built
/// [`generate_policies`] variant instead.
pub(crate) fn generate_policies_via_cargo(
    script_path: &Path,
    region: &str,
    account: &str,
) -> Option<Vec<Value>> {
