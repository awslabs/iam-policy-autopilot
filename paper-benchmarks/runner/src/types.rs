...
pub struct RoleInfo {
    ...
    /// AWS managed-policy ARNs attached to the role (empty for inline-only roles).
    /// Tracked so cleanup can detach them before deleting the role.
    pub policy_arns: Vec<String>,
}

/// How policies are supplied to [`crate::iam::create_execution_role`].
///
/// The benchmark pipeline needs to attach *either* generated inline policy
/// documents (autopilot / LLM output) *or* existing AWS managed-policy ARNs
/// (the set-cover result) to a temporary role, so the role creator accepts
/// both forms.
#[derive(Debug)]
pub enum RunPolicies {
    /// Inline IAM policy documents (e.g. autopilot output). Merged into a
    /// single inline role policy via `iam:PutRolePolicy`.
    InlineDocuments(Vec<Value>),
    /// AWS managed-policy ARNs — attached directly via `iam:AttachRolePolicy`
    /// without creating new policies.
    ManagedArns(Vec<String>),
}
