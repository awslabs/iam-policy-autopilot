...
**Recommended approach**: Use the `--service-hints` option to specify only the AWS services your application actually uses. This filters which SDK calls are analyzed, but the final policy may still include actions from other services if they're required by the operations you perform:
