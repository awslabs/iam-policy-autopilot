...
fn test_cli_help() {
    cli_command()
        ...
        .assert()
        .success()
        .stdout(predicate::str::contains("Unified tool that combines IAM policy generation"))
...
fn test_extract_sdk_calls_help() {
    extract_sdk_calls_command()
        ...
        .assert()
        .success()
        .stdout(predicate::str::contains("Extracts AWS SDK method calls from source code files"))
...
fn test_generate_policy_help() {
    generate_policy_command()
        ...
        .assert()
        .success()
        .stdout(predicate::str::contains("Generates complete IAM policy documents from source files"))
        .stdout(predicate::str::contains("--verbose"))
        .stdout(predicate::str::contains("--pretty"));
}
...
fn test_extract_sdk_calls_simplified_output() {
    ...
    if let Some(operations) = json.as_array() {
        for operation in operations {
            ...
            assert!(operation.get("name").is_some(), "Operation should have 'name' field");
            assert!(operation.get("possible_services").is_some(), "Operation should have 'possible_services' field");
            
            // possible_services should be an array
            let possible_services = operation.get("possible_services").unwrap();
            assert!(possible_services.is_array(), "possible_services should be an array");
        }
    }
}
...
fn test_extract_sdk_calls_full_output() {
    ...
    assert!(json.is_array(), "Output should be an array of method calls with metadata");
    
    // If there are method calls, verify they have metadata
    if let Some(methods) = json.as_array() {
        for method in methods {
            assert!(method.get("name").is_some(), "Method should have 'name' field");
            assert!(method.get("possible_services").is_some(), "Method should have 'possible_services' field");
            // With --full-output, methods should include metadata
            assert!(method.get("metadata").is_some(), "Method should have 'metadata' field with --full-output");
        }
    }
}
...
fn test_generate_policy_basic_functionality() {
    ...
    assert!(json.get("Policies").is_some(), "Output should have 'policies' field");
    let policies = json.get("Policies").unwrap();
    assert!(policies.is_array(), "Policies should be an array of IAM policies");
}
...
fn test_extract_sdk_calls_empty_file_simplified() {
    ...
    let stdout = String::from_utf8(output.get_output().stdout.clone()).unwrap();
    let json: Value = serde_json::from_str(&stdout).expect("Should produce valid JSON even for empty files");
    
    // Should be an empty array for simplified output
    assert!(json.is_array(), "Empty file should produce empty array");
    assert_eq!(json.as_array().unwrap().len(), 0, "Empty file should produce empty array");
}
...
fn test_extract_sdk_calls_empty_file_full_output() {
    ...
    assert!(json.is_array(), "Empty file should produce empty array even with --full-output");
    assert_eq!(json.as_array().unwrap().len(), 0, "Empty file should produce empty array");
}
...
fn test_generate_policy_empty_file() {
    ...
    let stdout = String::from_utf8(output.get_output().stdout.clone()).unwrap();
    let _json: Value = serde_json::from_str(&stdout).expect("Should produce valid JSON even for empty files");
}
...
fn test_comprehensive_real_files_extract_sdk_calls() {
    ...
    assert!(!operations.is_empty(), "Should find AWS SDK operations in test files");
    ...
    for operation in operations {
        ...
        let possible_services = operation.get("possible_services").unwrap();
        assert!(possible_services.is_array(), "possible_services should be an array");
        assert!(!possible_services.as_array().unwrap().is_empty(), "Should have at least one possible service");
    }
}
...
fn test_comprehensive_real_files_generate_policy() {
    ...
    assert!(policies_value.is_array(), "Policies should be an array of IAM policies");
    
    // Should have generated at least one policy from the real test files
    let policies = policies_value.as_array().unwrap();
    assert!(!policies.is_empty(), "Should generate IAM policies from test files");
    
    // Verify each policy has the expected IAM policy structure
    for policy_with_type in policies {
        assert!(policy_with_type.get("Policy").is_some(), "Should have 'Policy' field");
        assert!(policy_with_type.get("PolicyType").is_some(), "Should have 'PolicyType' field");
        
        let policy = policy_with_type.get("Policy").unwrap();
        assert!(policy.get("Version").is_some(), "Policy should have 'Version' field");
        assert!(policy.get("Statement").is_some(), "Policy should have 'Statement' field");
        ...
        for statement in statements.as_array().unwrap() {
            assert!(statement.get("Effect").is_some(), "Statement should have 'Effect' field");
            assert!(statement.get("Action").is_some(), "Statement should have 'Action' field");
            assert!(statement.get("Resource").is_some(), "Statement should have 'Resource' field");
        }
    }
}
...
fn test_generate_policy() {
    ...
    assert!(!policies_with_types.is_empty(), "Should generate at least one policy");
    
    let policy_with_type = &policies_with_types[0];
    let policy = policy_with_type.get("Policy").unwrap();
    assert_eq!(policy.get("Version").unwrap().as_str().unwrap(), "2012-10-17");
    ...
    let statements_with_conditions: Vec<_> = statements.iter().filter(|s| s.get("Condition").is_some()).collect();
    
    println!("{}", serde_json::to_string_pretty(policy).unwrap());
    assert!(!statements_with_conditions.is_empty(), "Should have at least one statement with conditions");
    ...
    for statement_with_condition in &statements_with_conditions {
        ...
        let kms_via_service = string_equals.get("kms:ViaService").unwrap().as_array().unwrap();
        
        // Verify that the ${region} placeholder was replaced with the actual region
        assert_eq!(kms_via_service.len(), 1, "Should have exactly one kms:ViaService condition value");
        ...
        assert!(condition_value.contains("us-east-1"),
            "Condition value '{}' should contain the actual region 'us-east-1'", condition_value);
        assert!(condition_value.contains("amazonaws.com"),
            "Condition value '{}' should contain 'amazonaws.com'", condition_value);
        assert!(!condition_value.contains("${region}"),
            r"Condition value '{}' should not contain the placeholder '${{region}}'", condition_value);
        
        // Get the actions for this statement
        let actions = statement_with_condition.get("Action").unwrap().as_array().unwrap();
        ...
        if action_strings.contains(&"kms:Decrypt") && action_strings.contains(&"kms:GenerateDataKey") {
            found_statement_with_both_actions = true;
            
            // Verify the expected resources are present for the statement with both actions
            let resources = statement_with_condition.get("Resource").unwrap().as_array().unwrap();
            let expected_resources = vec![
                "arn:aws:kms:us-east-1:123456789012:key/*",
            ];
            
            assert_eq!(resources.len(), expected_resources.len(), "Should have expected number of resources");
            for expected_resource in &expected_resources {
                assert!(resources.iter().any(|r| r.as_str().unwrap() == *expected_resource),
                    "Should contain resource: {}", expected_resource);
            }
        }
        
        // All statements with conditions should have at least kms:Decrypt
        assert!(action_strings.contains(&"kms:Decrypt"),
