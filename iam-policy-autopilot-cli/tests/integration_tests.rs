            ...
            for (_, condition_tests) in condition.as_object().unwrap() {
                for (_, condition_values) in condition_tests.as_object().unwrap() {
                    for condition_value in condition_values.as_array().unwrap() {
                        ...
                        // The condition value should contain the wild card region and partition instead of ${region}
                        if condition_value.starts_with("arn:") {
                            assert!(condition_value.contains("arn:aws-us-gov:"));
                            assert!(
                                condition_value.contains(":us-gov-east-1:123456789012:")
