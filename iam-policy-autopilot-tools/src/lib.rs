...
impl PolicyUploader {
    ...
    fn validate_policy_name(name: &str) -> UploaderResult<()> {
        if name.is_empty() {
            return Err(UploaderError::InvalidPolicyName(name.to_string(), "Name cannot be empty".to_string()));
        }
        ...
        if name.len() > max_len {
            return Err(UploaderError::InvalidPolicyName(name.to_string(), format!("Name cannot exceed {} characters", max_len)));
        }
        ...
        if !valid_chars.is_match(name) {
            return Err(UploaderError::InvalidPolicyName(
                name.to_string(),
                format!("Names must be alphanumeric and can include: + = , . @ _ - (see {})", NAME_SPEC_LINK)));
        }
    ...
    pub async fn upload_policies(
        ...
        for (index, policy) in policies.iter().enumerate() {
            match self.upload_single_policy_with_existing(&policy.policy, custom_name, &mut existing_policy_names).await {
                Ok(response) => {
...
mod tests {
    ...
    fn test_generate_policy_name_custom_with_existing() {
        let existing = vec![
            "MyPolicy_1".to_string(),
            "MyPolicy_3".to_string(),
        ];
    ...
    fn test_generate_policy_name_with_invalid_custom_name() {
        ...
        assert!(matches!(result.unwrap_err(), UploaderError::InvalidPolicyName(_, _)));
    }
